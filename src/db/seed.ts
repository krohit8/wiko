...
import db from ".";
import { articles, userSync } from "./Schema";
...
async function main() {
  try {
    ...
    await db.execute(sql.raw("TRUNCATE TABLE articles RESTART IDENTITY CASCADE;"));

    console.log("🔎 Querying existing users...");
    let users = await db
      .select({ id: userSync.id })
      .from(userSync)
      .orderBy(userSync.id);
    ...
    console.log("🍩 Manually inserting seed data...");
    const contents = [
    ...
    for (let i = 0; i < SEED_COUNT; i++) {
      ...
      dataToInsert.push({
        ...
        slug: `seed-article-${i + 1}-${Date.now()}-${Math.floor(Math.random() * 1000)}`,
        content,
        authorId,
        published: true,
      });
    ...
    await db.insert(articles).values(dataToInsert);
